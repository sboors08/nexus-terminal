...
test('Binance USD-M Futures client maps public responses to NEXUS contracts', async () => {
  ...
  const candles = await client.getCandles('SOLUSDT', '5m'); assert.equal(candles[0]?.open, 189); assert.equal(candles[0]?.tradesCount, 845); assert.ok(requests.includes('/fapi/v1/klines?symbol=SOLUSDT&interval=5m&limit=200'));
});
